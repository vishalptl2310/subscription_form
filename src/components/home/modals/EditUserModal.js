...
const EditUserModal = ({ editUserData, editUser, userList }) => {
  ...
  useEffect(() => {
    setName({ name: editUserData?.userName ?? "", error: "" });
    setSlotList(editUserData?.timeSlot ?? []);
  }, [editUserData]);
  ...
  const handleSubmit = () => {
    ...
    if (name.name.trim().length === 0) {
      ...
      userList.some((u, i) => u.userName === name.name && i !== editUserData.id)
    ) {
      setName({ ...name, error: "User already exist with this user name" });
      return;
    }
    if (!validateSlotFields() || !validateSlotTimings()) return;
    editUser({ userName: name.name, timeSlot: slotList }, editUserData.id);
  };
  return (
    <>
      <div
